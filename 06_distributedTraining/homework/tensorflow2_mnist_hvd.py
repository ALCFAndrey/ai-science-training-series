...
parser.add_argument('--metrics', default=1, type=int, help="Metrics FileName")
...
for ep in range(args.epochs):
    ...
    for batch, (images, labels) in enumerate(dataset.take(nstep)):
        ...
        loss_value_worker, acc_worker = training_step(images, labels)
        loss_value = hvd.allreduce(loss_value_worker, average=True)
        acc = hvd.allreduce(acc_worker, average=True)
    ...
    if hvd.rank() == 0: 
        checkpoint.save(checkpoint_dir)
np.savetxt("metrics_"+str(args.metrics)+".dat", np.array([metrics['train_acc'], metrics['train_loss'], metrics['valid_acc'], metrics['valid_loss'], metrics['time_per_epochs']]).transpose())
t1 = time.time()
